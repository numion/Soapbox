...
class SOAP_TBase(object):
    ...
    def _test_parse_message(self, xml, MessageType, schema):
        envelope = self.SOAP.Envelope.parsexml(self.ENVELOPE_XML.format('', xml))
    ...
    def _test_render_message(self, MessageType, PlaceType, schema):
        xml = self.SOAP.Envelope.response('GetWeatherByPlaceName',
            MessageType(Place=PlaceType(Name='Skypia')))
...
class SOAP11_Test(SOAP_TBase, unittest.TestCase):
    ...
    def test_parse_fault(self):
        envelope = self.SOAP.Envelope.parsexml(self.ENVELOPE_XML.format('', SOAP11_FAULT))
        code, message = self.SOAP.parse_fault_message(envelope.Body.Fault)
...
<soap:Fault>
    ...
    <soap:Reason>
        <soap:Text>Server was unable to process request.</soap:Text>
        <soap:Text>Go away.</soap:Text>
    </soap:Reason>
    <soap:Detail />
</soap:Fault>
"""
...
class SOAP12_Test(SOAP_TBase, unittest.TestCase):
    ...
    def test_parse_fault(self):
        envelope = self.SOAP.Envelope.parsexml(self.ENVELOPE_XML.format('', SOAP12_FAULT))
        code, message = self.SOAP.parse_fault_message(envelope.Body.Fault)
        self.assertEqual(message, 'Server was unable to process request.')
